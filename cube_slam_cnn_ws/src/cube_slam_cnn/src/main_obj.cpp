...
double addnumber;
bool whether_use_fqnet;
bool whether_use_3d_IoU;
bool whether_print_final_cuboid_details;
...
double fqnet(const cuboid *sample_obj, const cv::Mat& rgb_img, const Eigen::MatrixXd& bbox_2d, torch::jit::script::Module& module)
{
    cv::Mat draw_cuboid_img = rgb_img.clone();
    plot_image_with_cuboid_new(draw_cuboid_img, sample_obj);
    cv::Rect image_roi = cv::Rect(bbox_2d(0,0), bbox_2d(0,1), bbox_2d(0,2), bbox_2d(0,3));
    cv::Mat crop = draw_cuboid_img(image_roi);
		cv::resize(crop, crop, cv::Size(224, 224), cv::INTER_CUBIC);
    // cv::imshow("crop", crop);
    // cv::waitKey(0);
		torch::Tensor tensor_image2 = torch::from_blob(crop.data, { 1, crop.rows, crop.cols, 3 }, torch::kByte);
    tensor_image2 = tensor_image2.permute({ 0, 3, 1, 2});   
    torch::Tensor tensor_image = torch::ones({1, 3, crop.rows, crop.cols}, torch::dtype(torch::kByte));
    tensor_image[0][0] = tensor_image2[0][2];
    tensor_image[0][1] = tensor_image2[0][1];
    tensor_image[0][2] = tensor_image2[0][0];
    ...
    // tensor_image[0][0] = tensor_image[0][0].sub_(0.406).div_(0.225);
    // tensor_image[0][1] = tensor_image[0][1].sub_(0.456).div_(0.224);
    // tensor_image[0][2] = tensor_image[0][2].sub_(0.485).div_(0.229);
    
		// torch::Tensor output = module.forward({ tensor_image }).toTensor();
		std::vector<torch::jit::IValue> inputs;
    inputs.push_back(tensor_image);
    torch::Tensor output = module.forward(inputs).toTensor();
    double result = output.item().to<double>();
...
{  
    ...
    int total_frame_number = truth_frame_poses.rows();
		...
		std::string network_param = ros::package::getPath("cube_slam_cnn")+"/data/traced_resnet_model.pt";
		torch::jit::script::Module resnet18;
    if(whether_use_fqnet)
    {
        try {
            // Deserialize the ScriptModule from a file using torch::jit::load().
            resnet18 = torch::jit::load(network_param);
            std::cout << "successfully loaded network_param" << std::endl;
        }
        catch (const c10::Error& e) {
            std::cerr << "error loading the model\n";
            ROS_ERROR("error loading the network model, try other methods\n") ;
            // return -1;
        }
    }
    ...
    g2o::VertexCuboid* vCube;
		...
		// if (save_results_to_txt)// save all cube from all frame
		// {   
    ...
    for (int frame_index=0;frame_index<total_frame_number;frame_index++)
    {
	  ...
	  if (online_detect_mode)
	  {
	    ...
	    // if (save_results_to_txt)// save all edge from all frame
			// {
			// 	string save_edge = base_folder + "edge_detection/"+frame_index_c + "_online_edges.txt";
			// 	ofstream online_stream_edge;
			// 	online_stream_edge.open(save_edge.c_str());
			// 	online_stream_edge << all_lines_raw <<"\n";
			// 	online_stream_edge.close();
			// }	
        ...
        // std::cout << "all_lines_raw: " << all_lines_raw << std::endl;
		  ...
		  cuboid* detected_cube = frames_cuboids[0][0];  // NOTE this is a simple dataset, only one landmark
					...
					{  
						...
						Eigen::Matrix<double, 1, 10> cube_ground_truth = offline_pred_frame_objects.row(frame_index); // xyz, roty, dimension
						...
						if(whether_use_fqnet)
							sample_obj->skew_ratio = fqnet(sample_obj, raw_rgb_img, raw_2d_objs, resnet18);
						
						// double tmp = sample_obj->skew_ratio;
						// std::cout << frame_index << " sample: " << cube_idx <<" iou_predict: " << sample_obj->skew_ratio
						// 					<< " gt: " << tmp<< sample_obj->pos.transpose()<< std::endl;
					...
					if(whether_print_final_cuboid_details)
						std::cout << frame_index << " final: " << final_cube_idx <<" iou_predict: " << detected_cube->skew_ratio
											<< detected_cube->pos.transpose()<< std::endl;
		  ...
		  proposal_error = detected_cube->normalized_error;
			...
			// Eigen::Matrix<double, 8, 1> online_camera_data; 
			// if(save_results_to_txt && !detect_cuboid_obj.whether_sample_cam_roll_pitch )
			// online_camera_data << frame_index, curr_cam_pose_Twc.toVector();
		      ...
		      cube_local_meas = cube_ground_value.transform_to(curr_cam_pose_Twc_new);
					// std::cout << "final twc " << curr_cam_pose_Twc_new.toVector().transpose() << std::endl;
					// if(save_results_to_txt)
					// {online_camera_data << frame_index, curr_cam_pose_Twc_new.toVector();}
		  	}

				// if(save_results_to_txt)
				// {
				// 	Eigen::Matrix<double, 9, 1> online_cube_data;
				// 	online_cube_data<<frame_index,detected_cube->pos(0),detected_cube->pos(1),detected_cube->pos(2),detected_cube->rotY,
				// 					detected_cube->scale(0),detected_cube->scale(1),detected_cube->scale(2), detected_cube->normalized_error;  // xyz roll pitch yaw scale
				// 	online_stream_cube << online_cube_data.transpose() <<"\n";
				// 	online_stream_camera << online_camera_data.transpose() <<"\n";
				// } // save result to txt loop 
	  ...
	  else
	  {
		  ...
		  Vector9d cube_pose;cube_pose<<measure_data(1),measure_data(2),measure_data(3),0,0,measure_data(4),
						measure_data(5)-0.01,measure_data(6)-0.03,measure_data(7);  // xyz roll pitch yaw scale
		  cube_ground_value.fromMinimalVector(cube_pose);
		  ...
		  offline_cube_obs_row_id++; // switch to next row  NOTE at most one object one frame in this data
			...
			// // read local ground truth of previous objets
			// has_detected_cuboid = true;
			// VectorXd measure_data = offline_pred_frame_objects.row(frame_index);
			// Vector9d cube_pose;
		  ...
		  // cube_local_meas.fromMinimalVector(cube_pose);
			// if(save_results_to_txt)
			// {
			// 	g2o::SE3Quat cam_pose_Twc_tmp(truth_frame_poses.row(frame_index).tail<7>());
			// 	g2o::cuboid cube_ground_value = cube_local_meas.transform_from(cam_pose_Twc_tmp);
			// 	Vector9d cube_ground_vector = cube_ground_value.toMinimalVector();				
			// 	Vector9d cube_pose_save;
			// 	cube_pose_save<<frame_index,cube_ground_vector(0),cube_ground_vector(1),cube_ground_vector(2),
			// 			cube_ground_vector(5), cube_ground_vector(6),cube_ground_vector(7),cube_ground_vector(8), 0;  // xyz roll pitch yaw scale
			// 	online_stream_cube << cube_pose_save.transpose() <<"\n";
			// 	Eigen::Matrix<double, 8, 1> online_camera_data; 
			// 	online_camera_data << frame_index, cam_pose_Twc_tmp.toVector();
			// 	online_stream_camera << online_camera_data.transpose() <<"\n";
			// }

			// // read global ground truth of previous objets
			// has_detected_cuboid = true;
      ...
      // Matrix<double,4,4>  cube_local_pose = transToWolrd.inverse() * cube_global_pose;
			...
			// 	g2o::cuboid cube_ground_value = cube_local_meas.transform_from(cam_pose_Twc_tmp);
			// 	Vector9d cube_ground_vector = cube_ground_value.toMinimalVector();	
			// 	std::cout << "cube_local_vector: "<< cube_pose.transpose()<< std::endl;
			// 	std::cout << "cube_ground_vector: "<< cube_ground_vector.transpose()<< std::endl;
			// 	Vector9d cube_pose_save;
			// 	cube_pose_save<<frame_index,cube_ground_vector(0),cube_ground_vector(1),cube_ground_vector(2),
			// 			cube_ground_vector(5), cube_ground_vector(6),cube_ground_vector(7),cube_ground_vector(8), 0;  // xyz roll pitch yaw scale
			// 	online_stream_cube << cube_pose_save.transpose() <<"\n";
...
{
    ros::init(argc, argv, "object_slam_cnn");
    
    ros::NodeHandle nh;

    nh.param ("/base_folder", base_folder, ros::package::getPath("cube_slam_cnn")+"/data/");
    nh.param ("/online_detect_mode", online_detect_mode, false);
    nh.param ("/save_results_to_txt", save_results_to_txt, false);
    nh.param ("/whether_use_fqnet", whether_use_fqnet, false);
    nh.param ("/whether_use_3d_IoU", whether_use_3d_IoU, false);
		nh.param ("/whether_print_final_cuboid_details", whether_print_final_cuboid_details, false);
    ...
    // std::string pred_objs_txt = base_folder+"previous_obj_poses_gt_local.txt";  // saved cuboids in local ground frame.
    std::string pred_objs_txt = base_folder+"online_cubes.txt";  // saved cuboids in local ground frame.
