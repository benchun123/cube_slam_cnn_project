									 ...
									 MatrixXd all_lines_raw, std::vector<ObjectSet> &all_object_cuboids)
{
	...
	for (int object_id = 0; object_id < num_2d_objs; object_id++)
	{
		...
		// // added benchun 20201104 shuffle the data 
    // std::random_device rd;
    // std::shuffle(std::begin(obj_yaw_samples), std::end(obj_yaw_samples), rd);
		// 	for (int out_id = 0; out_id < obj_yaw_samples.size(); out_id++)
    // std::cout << "obj_yaw_samples: " << obj_yaw_samples[out_id] << std::endl;
		...
		for (int sample_down_expan_id = 0; sample_down_expan_id < down_expand_sample_all.size(); sample_down_expan_id++)
		{
			...
			// for (int out_id = 0; out_id < top_x_samples.size(); out_id++)
    	// std::cout << "top_x_samples: " << top_x_samples[out_id] << std::endl;
			...
			if (whether_sample_cam_roll_pitch)
			{
				...
				cam_roll_samples.push_back(cam_pose_raw.euler_angle(0)- 3.0 / 180.0 * M_PI);
				cam_roll_samples.push_back(cam_pose_raw.euler_angle(0)+ 3.0 / 180.0 * M_PI);
				cam_roll_samples.push_back(cam_pose_raw.euler_angle(0)- 6.0 / 180.0 * M_PI);
				cam_roll_samples.push_back(cam_pose_raw.euler_angle(0)+ 6.0 / 180.0 * M_PI);
				cam_pitch_samples.push_back(cam_pose_raw.euler_angle(1));
				cam_pitch_samples.push_back(cam_pose_raw.euler_angle(1));
				cam_pitch_samples.push_back(cam_pose_raw.euler_angle(1)- 3.0 / 180.0 * M_PI);
				cam_pitch_samples.push_back(cam_pose_raw.euler_angle(1)+ 3.0 / 180.0 * M_PI);
				cam_pitch_samples.push_back(cam_pose_raw.euler_angle(1)- 6.0 / 180.0 * M_PI);
				cam_pitch_samples.push_back(cam_pose_raw.euler_angle(1)+ 6.0 / 180.0 * M_PI);
				...
				for (int cam_pitch_id = 0; cam_pitch_id < cam_pitch_samples.size(); cam_pitch_id++)
					for (int obj_yaw_id = 0; obj_yaw_id < obj_yaw_samples.size(); obj_yaw_id++)
					{
						if (whether_sample_cam_roll_pitch)
						{
							...
							// std::cout<<"orientation:  "<<cam_roll_samples[cam_roll_id]
							// 			<<"  "<<cam_pitch_samples[cam_pitch_id]
							// 			<<"  "<<cam_pose_raw.euler_angle(2)<<std::endl;
						...
						all_vps.row(2) = vp_3;
								  ...
								  // std::cout<<"cam_pose.KinvR  "<<cam_pose.KinvR<<std::endl;
		...
		// {
		// 	for(int proposal_id=0; proposal_id<raw_obj_proposals.size(); proposal_id++ )
		// 	{
		// 		cv::Mat frame_all_cubes_img = rgb_img.clone();
		// 		std::cout << "frameid: "<<proposal_id<< std::endl;
		// 		plot_image_with_cuboid(frame_all_cubes_img, raw_obj_proposals[proposal_id]);
		// 		cv::imshow("frame_all_cubes_img", frame_all_cubes_img);
		// 		cv::waitKey(0);
		// 	}	
		// }
		for (int ii = 0; ii < raw_obj_proposals.size(); ii++) // use sorted index
		all_object_cuboids[object_id].push_back(raw_obj_proposals[ii]);
		// std::cout << "proposal number: " << raw_obj_proposals.size() << "\n";
		// std::cout << "camera_roll_delta " << all_object_cuboids[0][0]->camera_roll_delta << std::endl;

		// // comment out score or sort function benchun 20200914
